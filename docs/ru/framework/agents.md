...
Стандартно поставляемая реализация содержит OpenAI-like streaming протокол как некоторое
компромиссно-универсальное решение для совместимости. В зависимости от задач вашей системы этот модуль
стоит переработать под более удобный/лаконичный формат

**Конфигурация:** параметр `execution.streaming_generator` в конфиге агента или GlobalConfig. Значение резолвится из `StreamingGeneratorRegistry` по имени:

- `openai` (по умолчанию) — `OpenAIStreamingGenerator`: формат OpenAI SSE.
- `open_webui` — `OpenWebUIStreamingGenerator`: формат Open WebUI с блоками `<details>` для вызовов инструментов и результатов.

Свои генераторы: наследуйте `BaseStreamingGenerator`, задайте атрибут класса `name` — они автоматически регистрируются и доступны в конфиге по имени.

Получение стрима (после запуска агента):
```python
